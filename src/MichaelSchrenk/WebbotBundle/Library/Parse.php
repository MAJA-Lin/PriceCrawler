...
class Parse
{
    ...
    ***********************************************************************/
    public function split_string($string, $delineator, $desired, $type)
    {
        ...
        $lc_str = mb_strtolower($string);
    	$marker = mb_strtolower($delineator);

        # Return text BEFORE the delineator
        if ($desired == BEFORE) {
            if ($type == EXCL) {  // Return text ESCL of the delineator
                $split_here = mb_strpos($lc_str, $marker);
            } else {              // Return text INCL of the delineator
                $split_here = mb_strpos($lc_str, $marker)+mb_strlen($marker);
            }

            $parsed_string = mb_substr($string, 0, $split_here);
            ...
            $parsed_string =  mb_substr($string, $split_here, mb_strlen($string));
        }
    ...
    ***********************************************************************/
    ...
    DESCRIPTION:
            ...
            This public function is usful for returning an array that contains
